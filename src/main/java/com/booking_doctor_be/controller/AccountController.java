...
import com.booking_doctor_be.dto.OwnerDto;
...
import com.booking_doctor_be.repository.IRoleRepo;
import com.booking_doctor_be.service.IAccountService;
import com.booking_doctor_be.service.IOwnerService;
import com.booking_doctor_be.service.impl.DoctorService;
...
public class AccountController {
    ...
    public ResponseEntity<?> registerOwner(@RequestBody OwnerDto ownerDto) {
        Role role = new Role();
        role.setId(3);
        Account account = new Account(ownerDto.getEmail(),"123456",ownerDto.getEmail(),ownerDto.getPhone(), ownerDto.getAvatar(),"Đang hoạt động",role);
        account.setFirstname(ownerDto.getFirstname());
        account.setLastname(ownerDto.getLastname());
        Doctor doctor = ownerDto.getDoctor();
        Owner owner = ownerDto.getOwner();
        accountService.save(account);
        owner.setAccount(account);
        ownerService.save(owner);
        doctor.setOwner(account);
